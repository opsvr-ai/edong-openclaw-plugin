...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-DDryBwn_.js");
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-BKi0ecUD.js").then(({ outputRootHelp }) => {
		outputRootHelp();
	}).catch(handleError);
	return true;
}
function runMainOrRootHelp(argv) {
	...
	import("./run-main-DnNVpNel.js").then(({ runCli }) => runCli(argv)).catch((error) => {
