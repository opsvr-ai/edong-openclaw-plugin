...
const decoders = {
	async oga(buf) {
		...
		if (!decoder) {
			let { OggVorbisDecoder } = await import("./ogg-vorbis-qdOjEn4o.js");
	...
	async mp3(buf) {
		...
		if (!decoder) {
			const { MPEGDecoder } = await import("./mpg123-decoder-CZIHoljJ.js");
	...
	async flac(buf) {
		...
		if (!decoder) {
			const { FLACDecoder } = await import("./flac-loGYMYFc.js");
	...
	async opus(buf) {
		...
		if (!decoder) {
			const { OggOpusDecoder } = await import("./ogg-opus-decoder-DGtjIICL.js");
	...
	async wav(buf) {
		...
		if (!decode) {
			let module = await import("./node-wav-C3Je_xO3.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
	...
	async qoa(buf) {
		...
		if (!decode) decoders.qoa.decode = decode = (await import("./qoa-format-D-8khmtB.js")).decode;
		return buf && createBuffer(await decode(buf));
	}
};
