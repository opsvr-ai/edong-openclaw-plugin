...
import { downloadWecomEncryptedFile } from "./wecom-standalone-download.js";
...
export async function downloadAndSaveImages(params: {
  ...
  runtime: RuntimeEnv;
  /** URL 回调模式下可为 null，将使用 HTTP 下载 + 解密 */
      ...
      try {
        const result = await withTimeout(
          wsClient
            ...
            : downloadWecomEncryptedFile({ url: imageUrl, aesKey: imageAesKey, runtime }),
      ...
      try {
        const result = await withTimeout(
          wsClient
            ...
            : downloadWecomEncryptedFile({ url: fileUrl, aesKey: fileAesKey, runtime }),
